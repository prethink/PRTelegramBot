...
## Команды, адресованные боту в группе

В группе Telegram адресует команду конкретному боту, дописывая его username: нажатие на `/get_3` в списке команд отправляет `/get_3@my_bot`.

Фреймворк срезает этот суффикс раньше, чем текст прочитает что-либо ещё, поэтому в `args` окажется `3` и в личке, и в группе. Особенно это важно, когда username самого бота содержит разделитель: у бота с именем `cs2_server_bot` сырой текст `/get_3@cs2_server_bot`, разбитый по `_`, дал бы `3@cs2`, `server` и `bot` вместо единственного аргумента.

Суффикс срезается независимо от того, чьё имя в нём стоит, — группа с несколькими ботами разбирается сама: каждый отвечает на те команды, которые знает. Если хотите, чтобы ваш молчал, когда обращались к другому, сравните упоминание с `bot.BotName` в [собственной проверке перед выполнением](sobstvennye-proverki-pered-vypolneniem-komand.md).

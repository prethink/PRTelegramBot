    ...
    public static class Markup
    {
        ...
        public static InlineKeyboardMarkup PickMonthYear(in DateTime date, DateTimeFormatInfo dtfi)
        {
            var keyboardRows = new InlineKeyboardButton[][]
            {
                ...
                {
                    MenuGenerator.GetInlineButton(new InlineCallback<CallendarTCommand>(date.ToString("MMMM", dtfi), CallbackId.PickMonth, new CallendarTCommand(date))),
                    MenuGenerator.GetInlineButton(new InlineCallback<CallendarTCommand>(date.ToString("yyyy", dtfi), CallbackId.PickYear, new CallendarTCommand(date)))
                },
                new InlineKeyboardButton[]
                {
                    MenuGenerator.GetInlineButton(new InlineCallback<CallendarTCommand>("<<", CallbackId.ChangeTo, new CallendarTCommand(date))),
                    " "
                }
            };
        ...
        public static InlineKeyboardMarkup PickMonth(in DateTime date, DateTimeFormatInfo dtfi)
        {
            ...
            for (int month = 0, row = 0; month < 12; row++)
            {
                ...
                for (var j = 0; j < 3; j++, month++)
                {
                    ...
                    keyboardRow[j] = MenuGenerator.GetInlineButton(new InlineCallback<CallendarTCommand>(dtfi.MonthNames[month], CallbackId.YearMonthPicker, new CallendarTCommand(day)));
                }
        ...
        public static InlineKeyboardMarkup PickYear(in DateTime date, DateTimeFormatInfo dtfi)
        {
            ...
            for (int i = 0, row = 0; i < 12; row++)
            {
                ...
                for (var j = 0; j < 3; j++, i++)
                {
                    ...
                    keyboardRow[j] = MenuGenerator.GetInlineButton(new InlineCallback<CallendarTCommand>(day.ToString("yyyy", dtfi), CallbackId.YearMonthPicker, new CallendarTCommand(day)));
                }
